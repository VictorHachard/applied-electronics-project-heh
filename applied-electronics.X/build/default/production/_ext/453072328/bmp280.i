...
# 2 "<built-in>" 2
# 1 "../modules/bmp280.c" 2





# 1 "../modules/bmp280.h" 1
...
app_err_t bmp280_read_raw(int32_t *rawTemp, int32_t *rawPress)
{
    ...
    while (to--)
    {
        ...
        err = i2c2_write_read(0xEC, &reg, 1, 0xED, &status, 1);
        if (err != APP_OK) return err;
    ...
    reg = 0xF7;
    err = i2c2_write_read(0xEC, &reg, 1, 0xED, rbuf, 6);
    if (err != APP_OK) return err;
...
app_err_t bmp280_read_calibration(bmp280_calib_t *cal)
{
    ...
    err = i2c2_write_read(0xEC, &reg, 1, 0xED, buf, 24);
    if (err != APP_OK)
        return err;
