...
# 2 "<built-in>" 2
# 1 "../modules/rtc_ds1307.c" 2





# 1 "../modules/rtc_ds1307.h" 1
...
# 11 "../modules/rtc_ds1307.h" 2
# 29 "../modules/rtc_ds1307.h"
...
# 9 "../modules/rtc_ds1307.c" 2
# 25 "../modules/rtc_ds1307.c"
...
static app_err_t rtc_read_register(uint8_t reg, uint8_t* value) {
    return i2c2_write_read(0xD0, &reg, 1, 0xD1, value, 1);
}
...
static app_err_t rtc_write_register(uint8_t reg, uint8_t value) {
    ...
    buf[0] = reg;
    buf[1] = value;
    return i2c2_write(0xD0, buf, 2);
}
# 63 "../modules/rtc_ds1307.c"
app_err_t rtc_init(void) {
    app_err_t err;
    uint8_t seconds_reg;


    err = rtc_read_register(0x00, &seconds_reg);
    if (err != APP_OK) {
        return APP_EDEV;
    }
    ...
    if (seconds_reg & 0x80) {
        ...
        seconds_reg &= ~0x80;
        err = rtc_write_register(0x00, seconds_reg);
        if (err != APP_OK) {
            return err;
        }
    }
    ...
    err = rtc_write_register(0x07, 0x00);
    if (err != APP_OK) {
        return err;
    }
...
app_err_t rtc_set_time(const rtc_time_t* time) {
    ...
    err = rtc_write_register(0x00, 0x00);
    if (err != APP_OK) return err;


    err = rtc_write_register(0x01, dec_to_bcd(time->min));
    if (err != APP_OK) return err;


    err = rtc_write_register(0x02, dec_to_bcd(time->hour));
    if (err != APP_OK) return err;


    err = rtc_write_register(0x04, dec_to_bcd(time->day));
    if (err != APP_OK) return err;


    err = rtc_write_register(0x05, dec_to_bcd(time->month));
    if (err != APP_OK) return err;

    return APP_OK;
}
# 150 "../modules/rtc_ds1307.c"
app_err_t rtc_get_time(rtc_time_t* time) {
    app_err_t err;
    uint8_t buf[7];
    ...
    err = i2c2_write_read(0xD0, &reg_start, 1, 0xD1, buf, 7);
    if (err != APP_OK) {
        return err;
    }
